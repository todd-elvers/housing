...
- **CI:** `.github/workflows/ingest.yml` runs a free-source ingest hourly 8am–8pm PT against the shared Turso DB (single writer — laptops just read). Timing comes from a Cloudflare Worker cron (`infra/cron-worker`) that API-dispatches the workflow — GitHub's own `schedule` trigger drops ticks under load. Failures post to `DISCORD_ERROR_WEBHOOK`. The only GitHub secret is `AGE_KEY` (the CI age identity); all other secrets ride in the committed `.env.age`.
