...
//   [0] postingId delta (+ decode.minPostingId)   [1] postedDate delta (+ minPostedDate, s)
//   [3] price   [4] "locIdx:?:locIdx~lat~lon" → decode.locations[locIdx] = [type,"sfbay",sub]
//   tagged: [6,slug]  [5,beds,sqft]  [10,"$price"]  [4,...images]  [13,host]
// `postedToday=1&sort=date` = a NEW-listing feed for the whole Bay Area.
const SAPI =
...
function decodeItem(it: unknown[], decode: SapiResponse["data"]["decode"]): RawListing {
  ...
  const gm = String(it[4] ?? "").match(/^(\d+):\d+:(\d+)~(-?[\d.]+)~(-?[\d.]+)/);
  if (gm) {
    locIdx = Number(gm[2]);
    lat = Number(gm[3]);
    lon = Number(gm[4]);
